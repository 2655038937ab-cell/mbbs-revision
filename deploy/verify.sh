...
check_host() {
  ...
  local health
  health=$(curl -s --max-time 20 "https://$host/api/health" || echo "")
  case "$health" in
  ...
  local me
  me=$(curl -s --max-time 20 "https://$host/api/auth/me" || echo "")
  case "$me" in
  ...
  local hdr
  hdr=$(curl -s -o /dev/null -D - --max-time 25 -H 'Accept-Encoding: gzip' "https://$host/js/app.js" || echo "")
  case "$hdr" in
  ...
  local lite
  lite=$(curl -s --max-time 60 "https://$host/api/store/lessons?lite=1" || echo "")
  local n
  n=$(printf '%s' "$lite" | grep -o '"id"' | wc -l | tr -d ' ')
  if [ "${n:-0}" -gt 0 ]; then ok "课程列表 $n 门"; else bad "课程列表为空（数据没迁过来？）"; fi

  local counts
  counts=$(curl -s --max-time 60 "https://$host/api/store/cards?lite=1" | grep -o '"id"' | wc -l | tr -d ' ')
  if [ "${counts:-0}" -gt 0 ]; then note "闪卡 $counts 张"; fi

  local issuer
  issuer=$(curl -sIv --max-time 20 "https://$host/" 2>&1 | grep -i 'issuer:' | head -1 | tr -d '\r' | sed 's/.*issuer: //')
...
for h in "$@"; do
  ...
  [ -z "$ip" ] && { note "$h 解析不到 A 记录"; continue; }
